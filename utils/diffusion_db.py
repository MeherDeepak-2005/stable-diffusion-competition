...
def unzip_file(file: str, output_dir: str):
    """
    ...
    """
    part_number = os.path.basename(file).replace('.zip', '')
    with zipfile.ZipFile(file, 'r') as zip_ref:
        for member in zip_ref.namelist():
            ...
            if filename.endswith('.json'):
                target = open(os.path.join('F:/DiffusionDB/jsonFiles', f"{filename}"), "wb")
            else:
                target = open(os.path.join(output_dir, f"{filename}"), "wb")
            with source, target:
...
def unzip_all(files: list, output_dir: str):
    """
