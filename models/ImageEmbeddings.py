...
CLIP.encode_image -> Linear Layers -> Embeddings output (predicted prompt embeddings)
Actual prompt -> Inputs -> GPT2 Embeddings
...
class ImageEmbeddingGenerator(nn.Module):
    def __init__(self):
        super(ImageEmbeddingGenerator, self).__init__()

        self.gpt_embeddings = GPT2Model.from_pretrained('gpt2').wte
    ...
    def forward(self, x):
        ...
        x = self.linear(mul_x).to(torch.long)
        #
        # with torch.no_grad():
        #     embeddings = self.gpt_embeddings(x)
